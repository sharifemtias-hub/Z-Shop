...
let cart =
  JSON.parse(
    localStorage.getItem("zshop_cart") ||
    "[]"
  );

let wishlist =
  JSON.parse(
    localStorage.getItem("zshop_wishlist") ||
    "[]"
  );
...
function(id) {
  ...
  document.getElementById(
    ...
    ${
      ...
      `<p>
        📅 Expected:
        ${escapeHTML(product.expectedDate)}
      </p>`
      : ""
    }
...
function() {
  ...
  message.innerText =
    `✓ Coupon applied: ${settings.couponPercent}% off`;
...
async function() {
  ...
  try {
    ...
    /*
      Transaction ensures stock is checked
      and reduced atomically.
    */
  ...
  if(!orderId || !phone) {
    ...
    result.innerHTML =
      `<div class="status-timeline">
        Please enter both Order ID and phone number.
      </div>`;
  ...
  try {
    ...
    result.innerHTML = `
      ...
      <div class="status-timeline">
        ...
        <h3>
          Order ${escapeHTML(found.orderId)}
        </h3>
